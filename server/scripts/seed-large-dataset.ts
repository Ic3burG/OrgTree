...
const seed = db.transaction(() => {
  ...
  try {
      db.prepare(`INSERT OR IGNORE INTO users (id, email, password_hash, name, role, created_at, updated_at) 
      VALUES (?, ?, ?, ?, ?, datetime('now'), datetime('now'))`).run(userId, 'seed@example.com', 'hash', 'Seed User', 'admin');
  } catch (e) {
      // ignore
  }
