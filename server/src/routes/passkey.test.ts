...
describe('Passkey Routes', () => {
  ...
  describe('POST /api/auth/passkey/register/start', () => {
    it('should generate registration options', async () => {
      ...
      vi.mocked(passkeyService.generatePasskeyRegistrationOptions).mockResolvedValue(mockOptions as any);
      ...
      expect(passkeyService.generatePasskeyRegistrationOptions).toHaveBeenCalledWith(mockUser.id, 'test@example.com');
    });
    ...
    it('should handle errors', async () => {
      ...
      vi.mocked(passkeyService.generatePasskeyRegistrationOptions).mockRejectedValue(new Error('Failed options'));
  ...
  describe('POST /api/auth/passkey/register/finish', () => {
    it('should verify registration', async () => {
      ...
      vi.mocked(passkeyService.verifyPasskeyRegistration).mockResolvedValue(mockVerification as any);
  ...
  describe('POST /api/auth/passkey/login/start', () => {
    ...
    it('should return 404 if user not found by email', async () => {
      ...
      vi.mocked(passkeyService.generatePasskeyLoginOptions).mockResolvedValue({ challenge: 'any' } as any);
  ...
  describe('GET /api/auth/passkey/list', () => {
    it('should return safe passkey list', async () => {
      const mockPasskeys = [
        { id: 'pk1', created_at: '2024-01-01', last_used_at: '2024-01-02', backup_status: 1, secret_data: 'hidden' }
      ];
